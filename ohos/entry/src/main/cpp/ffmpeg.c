...
static int decode_interrupt_cb(void *ctx) { return received_nb_signals > atomic_load(&transcode_init_done); }
...
static void abort_codec_experimental(const AVCodec *c, int encoder) { exit_program(1); }
    ...
    if (do_benchmark_all) {
        ...
        if (fmt) {
            ...
            av_log(NULL, AV_LOG_INFO, "bench: %8" PRIu64 " user %8" PRIu64 " sys %8" PRIu64 " real %s \n",
                   t.user_usec - current_time.user_usec, t.sys_usec - current_time.sys_usec,
...
static double adjust_frame_pts_to_encoder_tb(OutputFile *of, OutputStream *ost, AVFrame *frame) {
    ...
    frame->pts =
        av_rescale_q(frame->pts, filter_tb, enc->time_base) - av_rescale_q(start_time, AV_TIME_BASE_Q, enc->time_base);
...
early_exit:
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "filter -> pts:%s pts_time:%s exact:%f time_base:%d/%d\n",
               ...
               (enc && frame) ? av_ts2timestr(frame->pts, &enc->time_base) : "NULL", float_pts,
...
static void update_video_stats(OutputStream *ost, const AVPacket *pkt, int write_vstats) {
    ...
    if (vstats_version <= 1) {
        ...
        fprintf(vstats_file, "out= %2d st= %2d frame= %5" PRId64 " q= %2.1f ", ost->file_index, ost->index,
                frame_number, ost->quality / (float)FF_QP2LAMBDA);
    }
...
static int encode_frame(OutputFile *of, OutputStream *ost, AVFrame *frame) {
    ...
    if (frame) {
        if (ost->enc_stats_pre.io)
            enc_stats_write(ost, &ost->enc_stats_pre, frame, NULL, ost->frames_encoded);
        ...
        if (debug_ts) {
            av_log(ost, AV_LOG_INFO,
                   ...
                   type_desc, av_ts2str(frame->pts), av_ts2timestr(frame->pts, &enc->time_base), enc->time_base.num,
                   enc->time_base.den);
        }
    }
    ...
    while (1) {
        ...
        if (ost->enc_stats_post.io)
            enc_stats_write(ost, &ost->enc_stats_post, NULL, pkt, ost->packets_encoded);
        ...
        if (debug_ts) {
            av_log(ost, AV_LOG_INFO,
                   ...
                   type_desc, av_ts2str(pkt->pts), av_ts2timestr(pkt->pts, &enc->time_base), av_ts2str(pkt->dts),
...
static void do_audio_out(OutputFile *of, OutputStream *ost, AVFrame *frame) {
    ...
    if (frame->pts == AV_NOPTS_VALUE)
        ...
        frame->pts = av_rescale_q(frame->pts, frame->time_base, enc->time_base) -
 ...
 */
static void video_sync_process(OutputFile *of, OutputStream *ost, AVFrame *next_picture, double duration,
...
static enum AVPictureType forced_kf_apply(void *logctx, KeyframeForceCtx *kf, AVRational tb, const AVFrame *in_picture,
    ...
    if (kf->index < kf->nb_pts && av_compare_ts(in_picture->pts, tb, kf->pts[kf->index], AV_TIME_BASE_Q) >= 0) {
        ...
        ff_dlog(NULL, "force_key_frame: n:%f n_forced:%f prev_forced_n:%f t:%f prev_forced_t:%f -> res:%f\n",
                ...
                kf->expr_const_values[FKF_PREV_FORCED_N], kf->expr_const_values[FKF_T],
    ...
    } else if (kf->type == KF_FORCE_SOURCE && in_picture->key_frame == 1 && !dup_idx) {
        goto force_keyframe;
...
static void do_video_out(OutputFile *of, OutputStream *ost, AVFrame *next_picture) {
    ...
    if (!next_picture) {
        ...
        nb_frames_prev = nb_frames =
            mid_pred(ost->last_nb0_frames[0], ost->last_nb0_frames[1], ost->last_nb0_frames[2]);
    } else {
        video_sync_process(of, ost, next_picture, duration, &nb_frames, &nb_frames_prev);
    }
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        ...
        while (1) {
            ...
            if (filtered_frame->pts != AV_NOPTS_VALUE) {
                ...
                ost->last_filter_pts = av_rescale_q(filtered_frame->pts, tb, AV_TIME_BASE_Q);
                filtered_frame->time_base = tb;

                if (debug_ts)
                    av_log(NULL, AV_LOG_INFO, "filter_raw -> pts:%s pts_time:%s time_base:%d/%d\n",
                           av_ts2str(filtered_frame->pts), av_ts2timestr(filtered_frame->pts, &tb), tb.num, tb.den);
            }
...
static void print_final_stats(int64_t total_size) {
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        ...
        data_size += s;
        if (ost->enc_ctx && (ost->enc_ctx->flags & (AV_CODEC_FLAG_PASS1 | AV_CODEC_FLAG_PASS2)) != AV_CODEC_FLAG_PASS1)
            pass1_used = 0;
    }
    ...
    av_log(
        ...
        video_size / 1024.0, audio_size / 1024.0, subtitle_size / 1024.0, other_size / 1024.0, extra_size / 1024.0);
    if (percent >= 0.0)
    ...
    for (i = 0; i < nb_input_files; i++) {
        ...
        for (j = 0; j < f->nb_streams; j++) {
            ...
            av_log(NULL, AV_LOG_VERBOSE, "  Input stream #%d:%d (%s): ", i, j, av_get_media_type_string(type));
            av_log(NULL, AV_LOG_VERBOSE, "%" PRIu64 " packets read (%" PRIu64 " bytes); ", ist->nb_packets,
                   ist->data_size);
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        for (j = 0; j < of->nb_streams; j++) {
            ...
            av_log(NULL, AV_LOG_VERBOSE, "  Output stream #%d:%d (%s): ", i, j, av_get_media_type_string(type));
            if (ost->enc_ctx) {
        ...
        if (!vid && ost->st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (enc && (enc->flags & AV_CODEC_FLAG_PSNR) &&
                ...
                for (j = 0; j < 3; j++) {
                    ...
                    av_bprintf(&buf_script, "stream_%d_%d_psnr_%c=%2.2f\n", ost->file_index, ost->index, type[j] | 32,
                               p);
                }
...
static void flush_encoders(void) {
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        ...
        if (!ost->initialized) {
            ...
            av_log(ost, AV_LOG_WARNING, "Finishing stream without any data written to it.\n");

            if (ost->filter && !fg->graph) {
                int x;
                for (x = 0; x < fg->nb_inputs; x++) {
                    InputFilter *ifilter = fg->inputs[x];
                    if (ifilter->format < 0 && ifilter_parameters_from_codecpar(ifilter, ifilter->ist->par) < 0) {
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt) {
    ...
    if (!ost->streamcopy_started && !(pkt->flags & AV_PKT_FLAG_KEY) && !ost->copy_initial_nonkeyframes)
        return;
    ...
    if (pkt->dts == AV_NOPTS_VALUE) {
        ...
        opkt->dts = av_rescale_delta(ist->st->time_base, pkt->dts, (AVRational){1, ist->par->sample_rate}, duration,
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output, int *decode_failed) {
    ...
    ist->next_pts += ((int64_t)AV_TIME_BASE * decoded_frame->nb_samples) / decoded_frame->sample_rate;
    ist->next_dts += ((int64_t)AV_TIME_BASE * decoded_frame->nb_samples) / decoded_frame->sample_rate;
    ...
    if (pkt && pkt->duration && ist->prev_pkt_pts != AV_NOPTS_VALUE && pkt->pts != AV_NOPTS_VALUE &&
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE)
        decoded_frame->pts = av_rescale_delta(
            decoded_frame_tb, decoded_frame->pts, (AVRational){1, decoded_frame->sample_rate},
            decoded_frame->nb_samples, &ist->filter_in_rescale_delta_last, (AVRational){1, decoded_frame->sample_rate});
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output, int64_t *duration_pts, int eof,
    ...
    if (*got_output && ret >= 0) {
        if (ist->dec_ctx->width != decoded_frame->width || ist->dec_ctx->height != decoded_frame->height ||
            ist->dec_ctx->pix_fmt != decoded_frame->format) {
            av_log(NULL, AV_LOG_DEBUG, "Frame parameters mismatch context %d,%d,%d != %d,%d,%d\n", decoded_frame->width,
                   decoded_frame->height, decoded_frame->format, ist->dec_ctx->width, ist->dec_ctx->height,
                   ist->dec_ctx->pix_fmt);
        }
    }
    ...
    *duration_pts = decoded_frame->duration;
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO,
               ...
               ist->st->index, av_ts2str(decoded_frame->pts), av_ts2timestr(decoded_frame->pts, &ist->st->time_base),
               best_effort_timestamp, av_ts2timestr(best_effort_timestamp, &ist->st->time_base),
               decoded_frame->key_frame, decoded_frame->pict_type, ist->st->time_base.num, ist->st->time_base.den);
    }
...
static int copy_av_subtitle(AVSubtitle *dst, AVSubtitle *src) {
    ...
    for (int i = 0; i < src->num_rects; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            size_t buf_size =
                src_rect->type == SUBTITLE_BITMAP && j == 1 ? AVPALETTE_SIZE : src_rect->h * src_rect->linesize[j];
...
static int fix_sub_duration_heartbeat(InputStream *ist, int64_t signal_pts) {
    ...
    if (!ist->fix_sub_duration || !prev_subtitle->num_rects || signal_pts <= prev_subtitle->pts)
        return 0;
...
static int transcode_subtitles(InputStream *ist, const AVPacket *pkt, int *got_output, int *decode_failed) {
    AVSubtitle subtitle;
    int ret = avcodec_decode_subtitle2(ist->dec_ctx, &subtitle, got_output, pkt);
...
static int send_filter_eof(InputStream *ist) {
    ...
    int64_t pts =
        av_rescale_q_rnd(ist->pts, AV_TIME_BASE_Q, ist->st->time_base, AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt, int no_eof) {
    ...
    if (!ist->saw_first_ts) {
        ist->first_dts = ist->dts = ist->st->avg_frame_rate.num
                                        ? -ist->dec_ctx->has_b_frames * AV_TIME_BASE / av_q2d(ist->st->avg_frame_rate)
                                        : 0;
        ist->pts = 0;
        if (pkt && pkt->pts != AV_NOPTS_VALUE && !ist->decoding_needed) {
            ist->first_dts = ist->dts += av_rescale_q(pkt->pts, ist->st->time_base, AV_TIME_BASE_Q);
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if (!repeating || !pkt || got_output) {
                if (pkt && pkt->duration) {
                    ...
                    int ticks =
                        ist->last_pkt_repeat_pict >= 0 ? ist->last_pkt_repeat_pict + 1 : ist->dec_ctx->ticks_per_frame;
                    duration_dts = ((int64_t)AV_TIME_BASE * ist->dec_ctx->framerate.den * ticks) /
    ...
    if (!ist->decoding_needed && pkt) {
        ...
        case AVMEDIA_TYPE_AUDIO:
            ...
            if (par->sample_rate) {
                ist->next_dts += ((int64_t)AV_TIME_BASE * par->frame_size) / par->sample_rate;
            } else {
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        if (!check_output_constraints(ist, ost) || ost->enc_ctx || (!pkt && no_eof))
            continue;
    ...
    if (ist->decoding_needed) {
        ...
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_DVB_SUBTITLE && (ist->decoding_needed & DECODING_FOR_OST)) {
            ...
            if (ist->decoding_needed & DECODING_FOR_FILTER)
                av_log(NULL, AV_LOG_WARNING,
                       "Warning using DVB subtitles for filtering and output at the same time is not fully supported, "
...
static int init_output_stream_streamcopy(OutputStream *ost) {
    ...
    if (!codec_tag) {
        ...
        if (!of->format->codec_tag || av_codec_get_id(of->format->codec_tag, par->codec_tag) == par->codec_id ||
    ...
    if (!ost->copy_prior_start) {
        ...
        if (copy_ts && ifile->start_time != AV_NOPTS_VALUE) {
            ost->ts_copy_start = FFMAX(ost->ts_copy_start, ifile->start_time + ifile->ts_offset);
        }
    }
...
static int init_output_stream_encode(OutputStream *ost, AVFrame *frame) {
    ...
    if (enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        if (ost->max_frame_rate.num && (av_q2d(ost->frame_rate) > av_q2d(ost->max_frame_rate) || !ost->frame_rate.den))
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (ost->bits_per_raw_sample)
            ...
            enc_ctx->bits_per_raw_sample =
                FFMIN(dec_ctx->bits_per_raw_sample, av_get_bytes_per_sample(enc_ctx->sample_fmt) << 3);
    ...
    case AVMEDIA_TYPE_SUBTITLE:
        ...
        if (dec_ctx && dec_ctx->subtitle_header) {
            ...
            if (!ost->enc_ctx->subtitle_header)
                return AVERROR(ENOMEM);
            memcpy(ost->enc_ctx->subtitle_header, dec_ctx->subtitle_header, dec_ctx->subtitle_header_size);
            ost->enc_ctx->subtitle_header_size = dec_ctx->subtitle_header_size;
        }
        if (ist && ist->dec->type == AVMEDIA_TYPE_SUBTITLE && enc_ctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
            int input_props = 0, output_props = 0;
            AVCodecDescriptor const *input_descriptor = avcodec_descriptor_get(ist->dec->id);
...
static int transcode_init(void) {
     ...
     */
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        if (ost->enc_ctx && (ost->st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO ||
...
dump_format:
    ...
    for (InputStream *ist = ist_iter(NULL); ist; ist = ist_iter(ist)) {
        for (int j = 0; j < ist->nb_filters; j++) {
            if (!filtergraph_is_simple(ist->filters[j]->graph)) {
                av_log(NULL, AV_LOG_INFO, "  Stream #%d:%d (%s) -> %s", ist->file_index, ist->st->index,
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        if (ost->attachment_filename) {
            /* an attached file */
            av_log(NULL, AV_LOG_INFO, "  File %s -> Stream #%d:%d\n", ost->attachment_filename, ost->file_index,
                   ost->index);
            continue;
        }
        ...
        av_log(NULL, AV_LOG_INFO, "  Stream #%d:%d -> #%d:%d", ost->ist->file_index, ost->ist->st->index,
               ost->file_index, ost->index);
        if (ost->enc_ctx) {
            ...
            av_log(NULL, AV_LOG_INFO, " (%s (%s) -> %s (%s))", in_codec_name, decoder_name, out_codec_name,
                   encoder_name);
        } else
        ...
        if (ost->filter && ost->last_filter_pts != AV_NOPTS_VALUE) {
            ...
            if (ost->last_mux_dts == AV_NOPTS_VALUE)
                av_log(ost, AV_LOG_DEBUG,
                       "cur_dts is invalid [init:%d i_done:%d finish:%d] (this is harmless if it occurs once at the "
                       "start per stream)\n",
...
static int check_keyboard_interaction(int64_t cur_time) {
    ...
    if (key == 'c' || key == 'C') {
        ...
        if (k > 0 && (n = sscanf(buf, "%63[^ ] %lf %255[^ ] %255[^\n]", target, &time, command, arg)) >= 3) {
            av_log(NULL, AV_LOG_DEBUG, "Processing command target:%s time:%f command:%s arg:%s", target, time, command,
                   arg);
            for (i = 0; i < nb_filtergraphs; i++) {
                FilterGraph *fg = filtergraphs[i];
                if (fg->graph) {
    ...
    for (int i = 0; i < ifile->nb_streams; i++) {
        ...
        if (ist->decoding_needed) {
            ...
            if (ist->par->codec_type == AVMEDIA_TYPE_AUDIO) {
                ...
                dur.stream_idx = i;
                dur.duration =
                    av_rescale_q(ist->nb_samples, (AVRational){1, ist->dec_ctx->sample_rate}, ist->st->time_base);
...
static void ts_discontinuity_detect(InputFile *ifile, InputStream *ist, AVPacket *pkt) {
    ...
    int64_t pkt_dts =
        av_rescale_q_rnd(pkt->dts, ist->st->time_base, AV_TIME_BASE_Q, AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);

    if (copy_ts && ist->next_dts != AV_NOPTS_VALUE && fmt_is_discont && ist->st->pts_wrap_bits < 60) {
        int64_t wrap_dts = av_rescale_q_rnd(pkt->dts + (1LL << ist->st->pts_wrap_bits), ist->st->time_base,
        ...
        if (fmt_is_discont) {
            if (FFABS(delta) > 1LL * dts_delta_threshold * AV_TIME_BASE ||
                ...
                av_log(NULL, AV_LOG_DEBUG,
                       ...
                       ist->file_index, ist->st->index, ist->st->id, av_get_media_type_string(ist->par->codec_type),
    ...
    } else if (ist->next_dts == AV_NOPTS_VALUE && !copy_ts && fmt_is_discont && ifile->last_ts != AV_NOPTS_VALUE) {
...
static void ts_discontinuity_process(InputFile *ifile, InputStream *ist, AVPacket *pkt) {
    int64_t offset = av_rescale_q(ifile->ts_offset_discont, AV_TIME_BASE_Q, ist->st->time_base);
    ...
    if ((ist->par->codec_type == AVMEDIA_TYPE_VIDEO || ist->par->codec_type == AVMEDIA_TYPE_AUDIO) &&
        pkt->dts != AV_NOPTS_VALUE)
...
static int process_input(int file_index) {
    ...
    if (ret < 0) {
        ...
        for (i = 0; i < ifile->nb_streams; i++) {
            ...
            for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
                if (ost->ist == ist && (!ost->enc_ctx || ost->enc_ctx->codec_type == AVMEDIA_TYPE_SUBTITLE)) {
                    OutputFile *of = output_files[ost->file_index];
                    of_output_packet(of, ost->pkt, ost, 1);
                }
            }
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO,
               "demuxer+ffmpeg -> ist_index:%d:%d type:%s pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s "
               "duration:%s duration_time:%s off:%s off_time:%s\n",
               ifile->index, pkt->stream_index, av_get_media_type_string(ist->par->codec_type), av_ts2str(pkt->pts),
               av_ts2timestr(pkt->pts, &ist->st->time_base), av_ts2str(pkt->dts),
               av_ts2timestr(pkt->dts, &ist->st->time_base), av_ts2str(pkt->duration),
               av_ts2timestr(pkt->duration, &ist->st->time_base), av_ts2str(input_files[ist->file_index]->ts_offset),
