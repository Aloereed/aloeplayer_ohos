...
class DownloadManager extends ChangeNotifier {
  ...
  DownloadManager.forTesting(
      ...
      required Future<FileService> Function(String serverId) openSource,
      MemberAccess? access})
      : access = access ?? MemberAccess.instance {
  ...
  Future<void> _load() async {
    if (Platform.operatingSystem == 'ohos') {
      _device.setMethodCallHandler((call) async {
        if (call.method == 'downloadBackgroundCanceled') {
          ...
          for (final task
              in tasks.where((t) => t.status == DownloadStatus.queued)) {
    ...
    final raw =
        (await SharedPreferences.getInstance()).getString('download.tasks.v1');
    if (raw != null) {
  ...
  Future<void> _persist() {
    ...
    _writes = _writes.catchError((_) {}).then((_) async {
      await (await SharedPreferences.getInstance())
          .setString('download.tasks.v1', snapshot);
    });
    return _writes;
  }

  /// Once accepted, tasks can finish or resume even after membership expires.
  Future<({int added, int skipped, int failed})> addBatch(
      ServerConfig config, List<FileItem> files) async {
    await access.require(MemberFeature.batchDownload);
    if (files.isEmpty ||
        files.length > 100 ||
        files.any((f) => f.isDirectory || f.size < 0)) {
  ...
  Future<void> add(ServerConfig config, FileItem file) async {
    ...
    if (file.isDirectory || !hasKnownFileSize(file))
      throw StateError('无法下载目录或未知大小的文件，请先刷新文件信息');
    final id = const Uuid().v4();
    final extension = path.posix.extension(file.path).toLowerCase();
    final audio = {
      '.mp3',
      '.flac',
      '.m4a',
    ...
    final directory = Directory(_directoryOverride ??
        '/storage/Users/currentUser/Download/com.aloereed.aloeplayer/${audio ? 'Audios' : 'Videos'}/Downloads');
    await directory.create(recursive: true);
    final downloadName =
        path.extension(file.name).isEmpty && extension.isNotEmpty
            ? '${file.name}$extension'
            : file.name;
    var safeName = path
    ...
    if (safeName.isEmpty || safeName == '.' || safeName == '..')
      safeName = 'download-$id';
    var destination = path.join(directory.path, safeName);
    if (await File(destination).exists() ||
        tasks.any((t) => t.destination == destination)) {
    ...
    try {
      if (Platform.operatingSystem == 'ohos') {
        try {
          ...
          await _device
              .invokeMethod<void>('requestDownloadNotificationPermission');
          final started = await _device
              .invokeMethod<bool>('startDownloadBackground')
              .timeout(const Duration(seconds: 10));
          if (started != true)
            throw StateError('Background task did not start');
          backgroundNotice = null;
        } catch (error) {
          backgroundNotice = downloadBackgroundFailure(error);
        }
        notifyListeners();
      }
      while (true) {
        final next =
            tasks.where((t) => t.status == DownloadStatus.queued).firstOrNull;
        if (next == null) break;
        _activeId = next.id;
      ...
      if (Platform.operatingSystem == 'ohos') {
        try {
          await _device
              .invokeMethod<void>('stopDownloadBackground')
              .timeout(const Duration(seconds: 10));
        } catch (_) {}
      }
      _running = false;
      ...
      if (tasks.any((t) => t.status == DownloadStatus.queued))
        unawaited(_pump());
    }
  }
  ...
  Future<void> _updateBackgroundProgress(DownloadTask task) async {
    if (Platform.operatingSystem != 'ohos' ||
        _updatingBackground ||
        DateTime.now().difference(_lastBackgroundProgress).inSeconds < 3)
      return;
    ...
    try {
      await _device.invokeMethod<void>('updateDownloadBackground', {
        'name': task.name,
        'progress': task.size > 0 ? (task.received * 100 ~/ task.size) : 0,
    ...
    try {
      ...
      if (source == null ||
          ...
          (task.modifiedMs != null &&
              source.modified?.millisecondsSinceEpoch != task.modifiedMs) ||
          (task.etag != null && fileEtag(source) != task.etag)) {
      ...
      if (Platform.operatingSystem == 'ohos') {
        final free = await const MethodChannel('aloeplayer/device-tools')
            .invokeMethod<int>('freeBytes');
        if (free != null && free < task.size - task.received + 32 * 1024 * 1024)
          throw StateError('存储空间不足');
      }
      if (task.status != DownloadStatus.downloading) return;
      writer = await part.open(mode: FileMode.append);
      if (task.received < task.size) {
        final start = task.received == 0 ? null : task.received;
        final stream = files is RevisionAwareFileService
            ? await (files as RevisionAwareFileService)
                .getFileStreamForRevision(source, start: start)
            : await files.getFileStream(task.remotePath, start: start);
        ...
        while (task.status == DownloadStatus.downloading &&
          ...
          if (task.received + iterator.current.length > task.size)
            throw StateError('服务器返回的文件大小不符');
          ...
          if (DateTime.now().difference(lastNotification).inMilliseconds >=
            ...
            notifyListeners();
            unawaited(_updateBackgroundProgress(task));
            if (DateTime.now().difference(checkpoint).inSeconds >= 2) {
              await _persist();
      ...
      if (identical(_activeSource, files)) _activeSource = null;
      if (task.status == DownloadStatus.canceled && await part.exists())
        await part.delete();
      await _persist();
      notifyListeners();
    }
