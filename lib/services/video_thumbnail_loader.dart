...
class VideoThumbnailLoader {
  ...
  VideoThumbnailLoader({required this.disk, required this.memory,
    required this.library, required this.decode});

  Future<Map<String, int>> _names() async {
    final names = <String, int>{};
    await for (final entry in library.list(recursive: true, followLinks: false)) {
      if (entry is File) names.update(path.basename(entry.path), (n) => n + 1, ifAbsent: () => 1);
    }
    return names;
  }
  ...
  Future<Uint8List?> load(File file) async {
    ...
    try {
      ...
      final target = uri?.scheme == 'file' && uri!.host.isEmpty ? File.fromUri(uri) : File(source);
    ...
    final key = ThumbnailCache.key(source, stat.size, stat.modified.millisecondsSinceEpoch);
    final cached = memory.get(key);
    if (cached != null) return cached;
    try {
      final bytes = await disk.read(key);
      if (bytes != null && bytes.isNotEmpty) { memory.put(key, bytes); return bytes; }
    } catch (_) { /* A read-only or unavailable cache is optional. */ }
    Uint8List? bytes;
    try { bytes = await decode(source); } catch (_) {}
    if (bytes == null || bytes.isEmpty) {
      try {
        final legacy = File(path.join(disk.directory.path, '${path.basename(file.path)}.jpg'));
        if (await legacy.exists() && !(await legacy.lastModified()).isBefore(stat.modified)) {
          final names = await (_legacyNames ??= _names());
          if (names[path.basename(file.path)] == 1) bytes = await legacy.readAsBytes();
        }
      } catch (_) { _legacyNames = null; }
    }
    if (bytes == null || bytes.isEmpty) return null;
    memory.put(key, bytes);
    try { await disk.write(key, bytes); } catch (_) {}
    return bytes;
  }
}
