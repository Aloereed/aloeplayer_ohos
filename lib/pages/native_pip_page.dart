...
class NativePipController {
  ...
  Future<void> seek(Duration position) async {
    await _channel
        ?.invokeMethod<void>('seek', {'positionMs': position.inMilliseconds});
  }
}
...
class NativePipPage extends StatefulWidget {
  ...
  const NativePipPage(
      ...
      this.onPosition});
  @override
  State<NativePipPage> createState() => _NativePipPageState();
}
...
class _NativePipPageState extends State<NativePipPage> {
  ...
  Future<void> _receive(MethodCall call) async {
    ...
    if (call.method == 'position' || call.method == 'closed') {
      ...
      _returnState = PipPlaybackResult((state['positionMs'] as num).round(),
          !_timerStopped && state['playing'] == true);
