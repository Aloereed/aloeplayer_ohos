...
class _NativeBackend implements SmbWorkerBackend {
  ...
  @override
  Future<bool> connect(Map<String, dynamic> options) => _service.connect(
      host: options['host'],
  ...
  @override
  Future<List<Libsmb2File>> list(String path) => _service.listFiles(path);
  @override
  Future<Libsmb2File> stat(String path) => _service.getFile(path);
  @override
  Future<Stream<Uint8List>> range(String path, int start, int? end) =>
      _service.getRangeStream(path, start: start, end: end);
  @override
  ...
  inbox.listen((dynamic message) {
    ...
    unawaited(serial.run(() async {
      ...
      try {
        ...
        switch (command) {
          case 'connect':
            value = await backend.connect(args);
            break;
          case 'list':
            value = await backend.list(args['path']);
            break;
          case 'stat':
            value = await backend.stat(args['path']);
            break;
          case 'open':
            if (readers.length >= 32) throw StateError('SMB 并发读取过多，请稍后重试');
            final stream =
                await backend.range(args['path'], args['start'], args['end']);
          ...
          case 'next':
            ...
            value = await reader.moveNext()
                ? TransferableTypedData.fromList([reader.current])
                : null;
            break;
          case 'close':
            await readers.remove(args['reader'])?.cancel();
            break;
          case 'disconnect':
            ...
            break;
          default:
            throw StateError('Unknown SMB operation');
        }
...
class SmbWorker {
  ...
  Future<void> _launch() async {
    ...
    events.listen((dynamic message) {
      if (!identical(_events, events)) return;
      if (message is SendPort) {
        ...
        _failed(StateError('SMB 后台连接已结束'));
      }
    });
    try {
      _isolate = await Isolate.spawn(_entry, events.sendPort,
  ...
  Future<Object?> _request(String command, Map<String, dynamic> args) async {
    if (!{'connect', 'disconnect'}.contains(command) && !isConnected)
      throw StateError('SMB 连接已关闭');
    final port = _port;
  ...
  Future<bool> connect(
      ...
      _connections.run(() async {
        await _disconnect();
        _closing = false;
        await (_starting = _launch());
        _starting = null;
        try {
          _connected = await _request('connect', {
                'host': host,
                'username': username,
                'password': password,
                'domain': domain,
  ...
  Future<List<Libsmb2File>> listFiles(String path) async =>
      (await _request('list', {'path': path}) as List).cast<Libsmb2File>();
  Future<Libsmb2File> getFile(String path) async =>
      await _request('stat', {'path': path}) as Libsmb2File;
  Future<Stream<Uint8List>> getFileStream(String path) =>
      getRangeStream(path, start: 0);
  Future<Stream<Uint8List>> getRangeStream(String path,
    ...
    return _range(path, start, end, _generation);
  }

  Stream<Uint8List> _range(
      String path, int start, int? end, int generation) async* {
    void checkConnection() {
      if (!isConnected || generation != _generation)
        throw StateError('SMB 连接已变化，请重新开始读取');
    }

    checkConnection();
    final reader =
        await _request('open', {'path': path, 'start': start, 'end': end})
            as int;
    try {
      while (true) {
        checkConnection();
      ...
      if (generation == _generation && _port != null && !_closing) {
        try {
          await _request('close', {'reader': reader});
        } catch (_) {}
      }
    }
  }
