...
class _SubtitleTrackDialogState extends State<SubtitleTrackDialog> {
  ...
  Widget build(BuildContext context) => StreamBuilder<List<SubtitleTrack>>(
      stream: widget.tracks,
      initialData: widget.initialTracks,
      builder: (context, snapshot) {
        final seen = <String>{};
        final tracks = [
          ...(snapshot.data ?? widget.initialTracks),
          ...widget.externalTracks
        ]
            .where((track) =>
                track.id != 'auto' && track.id != 'no' && seen.add(track.id))
            .toList();
                        ...
                        child: Scrollbar(
                            ...
                            child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final selected =
                                      widget.currentTrack.id == track.id;
                                  final title = track.id == 'auto'
                                      ? '自动选择字幕'
                                      : track.id == 'no'
                                          ? '无字幕'
                                          : (track.title?.trim().isNotEmpty ==
                                              ...
                                              : '字幕轨道 ${track.id}');
                                  final info = [
                                    if (index >= 2 && !track.uri && !track.data)
                                      '轨道 ${track.id}',
                                    if (track.language?.isNotEmpty == true)
                                      track.language!,
                                    if (track.codec?.isNotEmpty == true)
                                      track.codec!,
