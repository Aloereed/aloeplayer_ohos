"""Read-only SMB2 integration fixture. Binds ONLY to IPv4 loopback.
...
root = Path(__file__).resolve().parents[1] / 'build' / 'smb-loopback-data'
...
(root / 'server.json').write_text(json.dumps({'host': f'127.0.0.1:{port}', 'username': 'fixture',
    'password': 'fixture-password'}), encoding='utf-8')
print(f'SMB2 fixture on 127.0.0.1:{port}', flush=True)
server.start()
