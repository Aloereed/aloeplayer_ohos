...
$lines += @('', "检查时间：$((Get-Date).ToString('o'))；产物数：$verified；SHA-256 全量验证：$VerifyHashes。", '',
    ...
    '历史功能回退：34 月会员 IAP（需配套后端）；33 移动端标题搜索合并；32 视频 / 音频文件夹卡片尺寸对齐；31 底栏遮挡修复；30 鸿蒙风格视觉（有底部遮挡问题）；29 中文标题。第二轮回退：28 原生 PiP 竞态；27 缩略图重建与文件菜单；26 PiP / 画质交互；25 FFmpeg 后备缩略图；24 后台探测；23 主题 / 音频库；22 导航 / 视频库；21 FSR；20 导入说明；19 媒体来源；18 原生 PiP 页面；17 缩略图资源释放。', '',
